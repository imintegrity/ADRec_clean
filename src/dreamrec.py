...
class Diffu_xstart(nn.Module):
    ...
    def forward_cfg(self,c, x, t, mask_seq,mask_tgt,cfg_scale=1.0):
        ...
        x, t, mask_seq,mask_tgt =[i.repeat(2,*torch.ones(len(i.shape)-1,dtype=torch.int16).tolist()) for i in [x, t, mask_seq,mask_tgt]]
        half = x[: len(x) // 2]
        combined = torch.cat([half, half], dim=0)
        _,eps = self.forward(c,combined, t,mask_seq,mask_tgt)
...
class DreamRec(nn.Module):
    ...
    def p_mean_variance(self, rep_item, x_t, t, mask_seq,mask_tag):
        ...
        if self.cfg_scale==1:
            ...
            x_0 = self.net.forward_cfg(rep_item, x_t, self._scale_timesteps(t), mask_seq, mask_tag)
    ...
    def forward(self, item_rep, item_tag, mask_seq,mask_tag):
        ...
        x_t,t = self.independent_diffuse(item_tag, mask_tag, is_independent=False)
        # mask = torch.rand_like(mask_seq.shape[0]) > 0.9
        # item_rep = torch.where(mask.reshape(-1,1,1).repeat(1,*item_rep.shape[1:]),torch.zeros_like(item_rep),item_rep)

...
def cmdline_args():
    ...
    parser.add_argument('--item_alignment_mode', type=str, choices=['ce', 'topk_kd', 'pref_ratio', 'cosine_margin_ce'], help='Item-space alignment loss mode for x0_hat')
    parser.add_argument('--item_alignment_temperature', type=float, help='Temperature for normalized x0_hat-to-item cosine logits')
    parser.add_argument('--item_alignment_topk', type=int, help='Teacher top-K candidate size for local listwise distillation')
    parser.add_argument('--item_alignment_kd_temperature', type=float, help='Temperature for teacher-student top-K distillation')
    parser.add_argument('--item_alignment_teacher_source', type=str, choices=['main_ce_head'], help='Teacher source for top-K distillation')
    ...
    parser.add_argument('--prediction_target_mode', type=str, choices=['item_emb', 'pref_state'], help='Diffusion prediction target mode')
    parser.add_argument('--pref_teacher_topk', type=int, help='Teacher top-k size for preference-state prototype construction')
    parser.add_argument('--pref_teacher_temperature', type=float, help='Teacher temperature for preference-state prototype weights')
    parser.add_argument('--pref_mix_topk_alpha', type=float, help='Mixing weight for local top-k preference prototype')
    parser.add_argument('--pref_mix_stationary_beta', type=float, help='Mixing weight for stationary anchor in preference-state target')
    parser.add_argument('--generative_process_mode', type=str, choices=['diffusion', 'flow_matching'], help='Generative process mode')
    parser.add_argument('--flow_source_mode', type=str, choices=['stationary_anchor', 'stationary_anchor_blend_last'], help='Source prior mode for flow matching')
    parser.add_argument('--flow_source_hist_blend_rho', type=float, help='Small blend ratio for combining stationary anchor with last history state')
    parser.add_argument('--flow_num_steps', type=int, help='Number of Euler steps for flow-mode inference')
    parser.add_argument('--flow_time_schedule', type=str, choices=['linear'], help='Time discretization schedule for flow-mode inference')
    parser.add_argument('--flow_loss_weight', type=float, help='Weight for flow matching loss')
    parser.add_argument('--trajectory_consistency_mode', type=str, choices=['none', 'td_adjacent'], help='Trajectory consistency mode on top of diffusion baseline')
    parser.add_argument('--td_delta_step', type=int, help='Delta step between high and low diffusion timesteps for TD consistency')
    parser.add_argument('--td_loss_weight', type=float, help='Loss weight for trajectory consistency objective')
    parser.add_argument('--td_weighting_mode', type=str, choices=['none', 'snr'], help='Weighting scheme for TD consistency loss')
    parser.add_argument('--self_condition_mode', type=str, choices=['none', 'x0_prev'], help='Self-conditioning mode for reverse denoising')
    parser.add_argument('--self_condition_train_prob', type=float, help='Probability of enabling self-conditioning during training')
    parser.add_argument('--self_condition_dropout_prob', type=float, help='Dropout probability applied to detached self-conditioning signal')
    parser.add_argument('--self_condition_noise_std', type=float, help='Gaussian noise std applied to detached self-conditioning signal')
    parser.add_argument('--self_condition_fusion_mode', type=str, choices=['gated_add'], help='Fusion mode for self-conditioning input')
    parser.add_argument('--phase_target_mode', type=str, choices=['none', 'stationary_to_item'], help='Noise-phase target supervision mode')
    parser.add_argument('--phase_t_split', type=int, help='Timestep split between high-noise and low-noise phase targets')
    parser.add_argument('--phase_high_beta', type=float, help='Stationary-anchor mixing weight used only in the high-noise phase target')
    parser.add_argument('--use_positive_negative_guidance', type=str2bool, help='Enable minimal positive-negative guidance on top of stationary fixed alignment')
    parser.add_argument('--negative_condition_source', type=str, choices=['confusing_topk_from_main_ce'], help='Source used to build user-aware negative condition')
    parser.add_argument('--negative_condition_topk', type=int, help='Top-k confusing negatives used to aggregate negative condition')
    parser.add_argument('--png_guidance_scale', type=float, help='Inference-time positive-negative guidance scale')
    parser.add_argument('--gal_margin', type=float, help='Margin used by guidance alignment loss in cosine space')
    parser.add_argument('--gal_weight', type=float, help='Weight applied to guidance alignment loss')
